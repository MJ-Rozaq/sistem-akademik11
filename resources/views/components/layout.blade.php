...
<body>
    ...
    setColors(color);" :class="{ 'dark': isDark }">
        <div class="flex h-screen antialiased text-gray-900 bg-gray-100 dark:bg-dark dark:text-light">
            ...
            <div class="flex flex-col flex-1 min-h-screen overflow-x-hidden overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between p-2 border-b dark:border-primary-darker">
                        ...
                        <nav aria-label="Secondary" class="hidden space-x-2 md:flex md:items-center">
                            ...
                            <div class="relative" x-data="{ open: false }">
                                <button type="button" aria-haspopup="true" :aria-expanded="open ? 'true' : 'false'"
                                    class="my-5 p-2 transition-opacity duration-200 rounded-full dark:opacity-75 dark:hover:opacity-100 focus:outline-none focus:ring dark:focus:opacity-100"
                                    @click="open = !open; $nextTick(() => { if(open){ $refs.userMenu.focus() } })">
                                    <span class="sr-only">User menu</span>
                                    <img class="w-10 h-10 rounded-full" src="{{ asset('img/avatar5.png') }}"
                                        alt="Ahmed Kamel" />
                                </button>
                ...
                <!-- Main content -->
                <div class="flex items-start flex-1 px-4 pt-10 w-full">
                    <main class="w-full mt-10"> <!-- Ensure the main takes full width -->
                        {{ $slot }}
                    </main>
                </div>
    ...
    <script>
        ...
        function openEditModal(id, name, nip, kode_dosen, kelas_id) {
            document.getElementById('dosenId').value = id;
            document.getElementById('name_' + id).value = name;
            document.getElementById('nip_' + id).value = nip;
            document.getElementById('kode_dosen_' + id).value = kode_dosen;

            const kelasSelect = document.getElementById('kelas_id');
            kelasSelect.value = kelas_id;

            const form = document.getElementById('editDosenForm_' + id);
            form.action = "{{ route('kaprodi.dosens.update', '') }}" + '/' + id;
        ...
        function openEditKelasModal(id, name, jumlah) {
            document.getElementById('name_' + id).value = name;
            document.getElementById('jumlah_' + id).value = jumlah;
            document.getElementById('editKelasForm_' + id).action = "{{ route('kaprodi.kelas.update', '') }}" + '/' + id;
            document.getElementById('editKelasModal_' + id).classList.remove('hidden');
        }

        function closeEditKelasModal(id) {
            document.getElementById('editKelasModal_' + id).classList.add('hidden');
        }

        function validateForm() {
            const kelasSelect = document.getElementById('kelas_id');
            if (kelasSelect.value === '') {
                alert('Please select a class.'); // Show an alert if no class is selected
                return false; // Prevent form submission
            }
            return true; // Allow submission if a valid option is selected
        }
    </script>
</body>
